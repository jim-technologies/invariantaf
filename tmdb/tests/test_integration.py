"""Integration tests — verify invariant protocol wiring (descriptor, registration, projections)."""

import json
import urllib.request

import pytest

from tmdb_mcp.gen.tmdb.v1 import tmdb_pb2 as pb
from tests.conftest import DESCRIPTOR_PATH


class TestDescriptorAndRegistration:
    def test_descriptor_loads(self):
        from invariant import Server

        srv = Server.from_descriptor(DESCRIPTOR_PATH)
        assert srv is not None

    def test_all_tools_registered(self, server):
        assert len(server.tools) == 10

    def test_tool_names(self, server):
        expected = {
            "TMDBService.SearchMovies",
            "TMDBService.SearchTV",
            "TMDBService.GetMovie",
            "TMDBService.GetTVShow",
            "TMDBService.GetTrending",
            "TMDBService.GetMovieCredits",
            "TMDBService.GetMovieReviews",
            "TMDBService.GetPopularMovies",
            "TMDBService.GetTopRatedMovies",
            "TMDBService.DiscoverMovies",
        }
        actual = set(server.tools.keys())
        missing = expected - actual
        assert not missing, f"Missing tools: {missing}"
        assert expected.issubset(actual)

    def test_tools_have_descriptions(self, server):
        for name, tool in server.tools.items():
            assert tool.description, f"{name} has no description"
            assert len(tool.description) > 10, f"{name} description too short"

    def test_tools_have_input_schemas(self, server):
        for name, tool in server.tools.items():
            schema = tool.input_schema
            assert isinstance(schema, dict), f"{name} schema is not a dict"
            assert schema.get("type") == "object", f"{name} schema type != object"


class TestCLIProjection:
    def test_search_movies(self, server):
        result = server._cli(
            ["TMDBService", "SearchMovies", "-r", '{"query":"fight club"}']
        )
        assert "results" in result
        assert len(result["results"]) >= 1

    def test_search_tv(self, server):
        result = server._cli(
            ["TMDBService", "SearchTV", "-r", '{"query":"breaking bad"}']
        )
        assert "results" in result
        assert result["results"][0].get("name") == "Breaking Bad"

    def test_get_movie(self, server):
        result = server._cli(
            ["TMDBService", "GetMovie", "-r", '{"id":550}']
        )
        assert result.get("title") == "Fight Club"
        assert result.get("id") == 550

    def test_get_tv_show(self, server):
        result = server._cli(
            ["TMDBService", "GetTVShow", "-r", '{"id":1396}']
        )
        assert result.get("name") == "Breaking Bad"

    def test_get_trending(self, server):
        result = server._cli(["TMDBService", "GetTrending"])
        assert "results" in result
        assert len(result["results"]) == 2

    def test_get_popular_movies(self, server):
        result = server._cli(["TMDBService", "GetPopularMovies"])
        assert "results" in result
        assert len(result["results"]) >= 1

    def test_get_top_rated_movies(self, server):
        result = server._cli(["TMDBService", "GetTopRatedMovies"])
        assert "results" in result

    def test_discover_movies(self, server):
        result = server._cli(["TMDBService", "DiscoverMovies"])
        assert "results" in result

    def test_unknown_method(self, server):
        with pytest.raises(Exception, match="(?i)unknown service/method"):
            server._cli(["TMDBService", "DoesNotExist"])

    def test_help(self, server):
        result = server._cli(["--help"])
        assert "TMDBService" in result
        assert "SearchMovies" in result

    def test_no_args_shows_usage(self, server):
        result = server._cli([])
        assert "Usage:" in result


class TestHTTPProjection:
    @pytest.fixture(autouse=True)
    def start_http(self, server):
        self.port = server._start_http(0)
        yield
        server._stop_http()

    def _post(self, path, body=None):
        data = json.dumps(body or {}).encode()
        req = urllib.request.Request(
            f"http://localhost:{self.port}{path}",
            data=data,
            headers={"Content-Type": "application/json"},
        )
        return json.loads(urllib.request.urlopen(req).read())

    def test_search_movies(self):
        result = self._post(
            "/tmdb.v1.TMDBService/SearchMovies",
            {"query": "fight club"},
        )
        assert "results" in result

    def test_get_trending(self):
        result = self._post("/tmdb.v1.TMDBService/GetTrending")
        assert "results" in result

    def test_get_popular_movies(self):
        result = self._post("/tmdb.v1.TMDBService/GetPopularMovies")
        assert "results" in result

    def test_get_top_rated_movies(self):
        result = self._post("/tmdb.v1.TMDBService/GetTopRatedMovies")
        assert "results" in result

    def test_404_unknown_route(self):
        with pytest.raises(urllib.error.HTTPError) as exc_info:
            self._post("/unknown.Service/Method")
        assert exc_info.value.code == 404


class TestMCPProjection:
    """Test the actual MCP JSON-RPC protocol over stdio."""

    @staticmethod
    def _mcp_request(msg_id, method, params=None):
        msg = {"jsonrpc": "2.0", "id": msg_id, "method": method}
        if params is not None:
            msg["params"] = params
        return json.dumps(msg)

    @staticmethod
    def _run_mcp_session(messages: list[str]) -> list[dict]:
        import subprocess
        import sys

        stdin_data = "\n".join(messages) + "\n"

        script = f"""
import sys
from pathlib import Path
from unittest.mock import MagicMock

sys.path.insert(0, str(Path({DESCRIPTOR_PATH!r}).parent / "src"))

from tmdb_mcp.gen.tmdb.v1 import tmdb_pb2 as pb
from tmdb_mcp.service import TMDBService
from invariant import Server

# Build mocked service.
http = MagicMock()
def mock_get(url, params=None):
    resp = MagicMock()
    resp.raise_for_status = MagicMock()
    if "/search/movie" in url:
        resp.json.return_value = {{"page": 1, "total_results": 1, "total_pages": 1,
            "results": [{{"id": 550, "title": "Fight Club", "overview": "An insomniac...",
            "release_date": "1999-10-15", "vote_average": 8.4, "vote_count": 25000,
            "popularity": 50.5, "poster_path": "/poster.jpg", "backdrop_path": "/backdrop.jpg",
            "genre_ids": [18, 53], "original_language": "en", "adult": False}}]}}
    elif "/trending/" in url:
        resp.json.return_value = {{"page": 1, "total_results": 1, "total_pages": 1,
            "results": [{{"id": 550, "title": "Fight Club", "name": "", "media_type": "movie",
            "overview": "An insomniac...", "release_date": "1999-10-15", "first_air_date": "",
            "vote_average": 8.4, "poster_path": "/poster.jpg", "popularity": 50.5}}]}}
    elif "/movie/popular" in url:
        resp.json.return_value = {{"page": 1, "total_results": 1, "total_pages": 1,
            "results": [{{"id": 550, "title": "Fight Club", "overview": "An insomniac...",
            "release_date": "1999-10-15", "vote_average": 8.4, "vote_count": 25000,
            "popularity": 50.5, "poster_path": "/poster.jpg", "backdrop_path": "/backdrop.jpg",
            "genre_ids": [18, 53], "original_language": "en", "adult": False}}]}}
    elif "/movie/top_rated" in url:
        resp.json.return_value = {{"page": 1, "total_results": 1, "total_pages": 1,
            "results": [{{"id": 278, "title": "The Shawshank Redemption", "overview": "Framed...",
            "release_date": "1994-09-23", "vote_average": 8.7, "vote_count": 23000,
            "popularity": 70.1, "poster_path": "/poster2.jpg", "backdrop_path": "/backdrop2.jpg",
            "genre_ids": [18, 80], "original_language": "en", "adult": False}}]}}
    else:
        resp.json.return_value = {{}}
    return resp

http.get = MagicMock(side_effect=mock_get)

svc = TMDBService.__new__(TMDBService)
svc._http = http
svc._api_key = "fake-key"

server = Server.from_descriptor({DESCRIPTOR_PATH!r}, name="test-tmdb", version="0.0.1")
server.register(svc)
server.serve(mcp=True)
"""
        proc = subprocess.run(
            [sys.executable, "-c", script],
            input=stdin_data,
            capture_output=True,
            text=True,
            timeout=10,
        )

        responses = []
        for line in proc.stdout.strip().split("\n"):
            if line.strip():
                responses.append(json.loads(line))
        return responses

    def test_initialize(self):
        responses = self._run_mcp_session([
            self._mcp_request(0, "initialize", {
                "protocolVersion": "2024-11-05",
                "capabilities": {},
                "clientInfo": {"name": "test", "version": "1.0"},
            }),
        ])
        assert len(responses) == 1
        assert responses[0]["result"]["protocolVersion"] == "2024-11-05"
        assert responses[0]["result"]["serverInfo"]["name"] == "test-tmdb"

    def test_tools_list(self):
        responses = self._run_mcp_session([
            self._mcp_request(0, "initialize", {}),
            self._mcp_request(1, "tools/list", {}),
        ])
        tools = responses[1]["result"]["tools"]
        assert len(tools) >= 10
        names = {t["name"] for t in tools}
        assert "TMDBService.SearchMovies" in names
        assert "TMDBService.GetTrending" in names
        assert "TMDBService.GetPopularMovies" in names

    def test_tool_call_search_movies(self):
        responses = self._run_mcp_session([
            self._mcp_request(0, "initialize", {}),
            self._mcp_request(1, "tools/call", {
                "name": "TMDBService.SearchMovies",
                "arguments": {"query": "fight club"},
            }),
        ])
        content = responses[1]["result"]["content"]
        result = json.loads(content[0]["text"])
        assert "results" in result
        assert result["results"][0].get("title") == "Fight Club"

    def test_tool_call_get_trending(self):
        responses = self._run_mcp_session([
            self._mcp_request(0, "initialize", {}),
            self._mcp_request(1, "tools/call", {
                "name": "TMDBService.GetTrending",
                "arguments": {},
            }),
        ])
        result = json.loads(responses[1]["result"]["content"][0]["text"])
        assert "results" in result
        assert result["results"][0].get("title") == "Fight Club"

    def test_tool_call_get_popular_movies(self):
        responses = self._run_mcp_session([
            self._mcp_request(0, "initialize", {}),
            self._mcp_request(1, "tools/call", {
                "name": "TMDBService.GetPopularMovies",
                "arguments": {},
            }),
        ])
        result = json.loads(responses[1]["result"]["content"][0]["text"])
        assert "results" in result

    def test_unknown_tool(self):
        responses = self._run_mcp_session([
            self._mcp_request(0, "initialize", {}),
            self._mcp_request(1, "tools/call", {
                "name": "DoesNotExist",
                "arguments": {},
            }),
        ])
        resp = responses[1]
        assert "error" in resp or resp.get("result", {}).get("isError") is True

    def test_ping(self):
        responses = self._run_mcp_session([
            self._mcp_request(0, "initialize", {}),
            self._mcp_request(1, "ping", {}),
        ])
        assert responses[1]["result"] == {}

    def test_unknown_method(self):
        responses = self._run_mcp_session([
            self._mcp_request(0, "initialize", {}),
            self._mcp_request(1, "unknown/method", {}),
        ])
        assert "error" in responses[1]
        assert responses[1]["error"]["code"] == -32601

    def test_notification_ignored(self):
        """Notifications (no id) should not produce a response."""
        responses = self._run_mcp_session([
            self._mcp_request(0, "initialize", {}),
            # notification — no id field
            json.dumps({"jsonrpc": "2.0", "method": "notifications/initialized"}),
            self._mcp_request(2, "ping", {}),
        ])
        # Should only get responses for id=0 and id=2, not the notification.
        ids = [r.get("id") for r in responses]
        assert 0 in ids
        assert 2 in ids
        assert len(responses) == 2


class TestInterceptor:
    def test_interceptor_fires(self, server):
        calls = []

        def logging_interceptor(request, context, info, handler):
            calls.append(info.full_method)
            return handler(request, context)

        server.use(logging_interceptor)
        server._cli(["TMDBService", "GetPopularMovies"])
        assert len(calls) == 1
        assert calls[0] == "/tmdb.v1.TMDBService/GetPopularMovies"

    def test_interceptor_chain_order(self, server):
        order = []

        def interceptor_a(request, context, info, handler):
            order.append("A-before")
            resp = handler(request, context)
            order.append("A-after")
            return resp

        def interceptor_b(request, context, info, handler):
            order.append("B-before")
            resp = handler(request, context)
            order.append("B-after")
            return resp

        server.use(interceptor_a)
        server.use(interceptor_b)
        server._cli(["TMDBService", "GetTrending"])
        assert order == ["A-before", "B-before", "B-after", "A-after"]
