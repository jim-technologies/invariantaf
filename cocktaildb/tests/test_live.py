...
def categories(live_server):
    ...
    result = asyncio.run(live_server._cli(["CocktailDBService", "ListCategories"]))
...
def glasses(live_server):
    ...
    result = asyncio.run(live_server._cli(["CocktailDBService", "ListGlasses"]))
...
class TestLiveRandom:
    def test_get_random_cocktail(self, live_server):
        result = asyncio.run(live_server._cli(["CocktailDBService", "GetRandomCocktail"]))
        assert "cocktail" in result
        cocktail = result["cocktail"]
...
class TestLiveLists:
    ...
    def test_list_ingredients(self, live_server):
        result = asyncio.run(live_server._cli(["CocktailDBService", "ListIngredients"]))
